...
class Metadata(dict):
    ...
    def validate_value(val):
        ...
        proper type but exceeds range limit for Topl metadata.
        """
        if isinstance(val, (str, bytes, bytearray)):
            if len(val) > 127 :
                raise ValueError(
                    "The string {} is too long ({:d} > 127)".format(val, len(val))
                )
            ...
            if len(str(val)) > 127:
                raise ValueError(
                    "The collection {} is too long ({:d} > 127)".format(val, len(str(val)))
                )
        else:
            raise TypeError(
                "Metadata values must be of int, str, bytes, bytearray, basic python collections or another Metadata instances, not {}".format(
                    str(type(val))
                )
            )
        return val
    ...
    def deserialize(txdata):
        """
        Deserializes transaction metadata :class: string and returns :class:`Metadata` instance.
