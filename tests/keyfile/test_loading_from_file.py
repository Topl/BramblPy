...
from brambl.keyfile.keyfile import create_keyfile_json, load_keyfile, extract_key_from_keyfile

PRIVATE_KEY = b58decode('4vJ9JU1bJJE96FWSJKvHsmmFADCg4gpZQff4P3bkLKi')
PASSWORD = 'test'


@pytest.fixture(params=['scrypt'])
def keyfile_json(request):
    _keyfile_json = create_keyfile_json(PRIVATE_KEY, PASSWORD, kdf=request.param, iterations=2, network_prefix=0x40,
...
def keyfile_path(tmpdir, keyfile_json):
    ...
    _keyfile_path.write(jsons.dumps(keyfile_json))
